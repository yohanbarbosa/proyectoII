...
export default function VentasModule() {
  ...
  const [expanded, setExpanded] = useState<
    Record<number, DetalleVenta[] | null>
  >({});
    ...
    <AppLayout>
      <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
        <PageHeader
          ...
          actions={
            <span
              className="text-[10px] font-semibold text-emerald-400 bg-emerald-500/10
                  ...
                  {rows.map((r) => (
                    <>
                      ...
                      <tr
                        ...
                        className="border-b border-zinc-800/50 hover:bg-(--color-hover) cursor-pointer transition-colors group"
                      >
                        <Td>
                          <Tag>{r.id_venta}</Tag>
                        </Td>
                        ...
                        <Td>
                          ...
                          ${
                            expanded[r.id_venta] !== undefined
                              ...
                              : "bg-(--color-bg-secondary) border-zinc-700 text-zinc-400 group-hover:border-zinc-600 group-hover:text-zinc-300"
                          }`}
                          >
                            {expanded[r.id_venta] !== undefined ? (
                              <>
                      ...
                      {expanded[r.id_venta] !== undefined && (
                        <tr
                          key={`detail-${r.id_venta}`}
                          className="bg-(--color-bg-secondary) border-b border-zinc-800/50"
                        >
                                ...
                                <thead>
                                  <tr className="border-b border-zinc-800/50">
                                    {[
                                      ...
                                      <th
                                        key={h}
                                        className="text-left pb-2 pr-6 text-xs font-semibold text-zinc-500 uppercase tracking-wider"
                                      >
                                        {h}
                                      </th>
                                    ))}
                                ...
                                <tbody>
                                  {expanded[r.id_venta]!.map((d) => (
                                    ...
                                    >
                                      ...
                                      <td className="py-2 pr-6 text-sm text-zinc-400">
                                        {d.cantidad}
                                      </td>
                                      <td className="py-2 pr-6 text-sm text-amber-400">
